...
public partial class UI_CorporateDeclarationDetailsCash : System.Web.UI.Page
{
        ...
        if (!IsPostBack)
        {
            ...
            if (bookCloserCorporateDec.Rows.Count > 0)
            {
                ...
                if (bookCloserDetails.Rows.Count > 0)
                {
                    ...
                    q = (" select * from (SELECT     FUND.F_CD,FUND.F_NAME, COMP.COMP_CD,COMP.COMP_NM,TRUNC(PFOLIO_BK.TOT_NOS) AS TOT_NOS ,CORPORATE_DEC.FY,CORPORATE_DEC.BOOK_CL_ID,CORPORATE_DEC.TYPE ,CORPORATE_DEC.RECORD_DT, CORPORATE_DEC.AGM,CORPORATE_DEC.APPR_DT, " +
             "   COMP.FC_VAL, CORPORATE_DEC.QUANTITY, COMP.FC_VAL * CORPORATE_DEC.QUANTITY / 100 AS DIVIDEND_PER_SHARE,   " +
              "  PFOLIO_BK.TOT_NOS * COMP.FC_VAL / 100 * CORPORATE_DEC.QUANTITY AS GROSS_DIVIDEND,  " +
                     "       PFOLIO_BK.TOT_NOS * COMP.FC_VAL / 100 * CORPORATE_DEC.QUANTITY * .2  AS TAX,   " +
                            "    PFOLIO_BK.TOT_NOS * COMP.FC_VAL / 100 * CORPORATE_DEC.QUANTITY * .8 " +
                                   "     AS NET_DIVIDEND,TO_CHAR(SYSDATE, 'MM-DD-YYYY') As PDATE  FROM    " +
                    ...
                    if (leftCheckBox.Checked)
                    {
                        ...
                        if (declarationType == "C")
                        {
                           ...
                           string accSchema = getFundSchema(fund_code);
                            ...
                            string accVoucherNo = SaveSaleUniAccountVoucher(accSchema, totalShare, grossDividend, comp_name, diviRatePerShare, fy, fund_code, agmDate, LoginName);
    ...
    public string SaveSaleUniAccountVoucher(string accountSchema, long totalShare, decimal totalDivAmt, string comp_name, decimal diviRate,string fy,int fund_code,DateTime agmDate, string LoginName)
    {
        ...
        try
        {
            ...
            htInsert.Add("REMARKS", " Div. Receivable from "+comp_name+" for fy:"+fy+" of Share:"+ totalShare + " @"+ diviRate  );
